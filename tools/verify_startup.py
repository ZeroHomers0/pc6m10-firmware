...
验证点：
  1) firmware.bin 内嵌 data_image（LMA=0xca4c, 0x213C 字节）是否 == docs/_data_image.bin
...
def main():
    ...
    img = open('docs/_data_image.bin', 'rb').read()

    print('== firmware.bin 大小:', hex(len(fw)))
    print('== _data_image.bin 大小:', len(img), '(=0x%x)' % len(img))
    ...
    m1 = embedded == img
    ok &= m1
    print('\n[1] firmware.bin@0x%x 内嵌 data_image == docs/_data_image.bin :' % RAM_IMAGE_LMA,
