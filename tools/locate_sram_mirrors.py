...
# 输出: docs/_sram_mirrors.txt (UTF-8)
#
...
#    定位方法：见 tools/decompress_dump.py（IAR 压缩 .data 流解压，精确收敛）
# 2. flash 高位区 0x36000-0x3FFF0 全 0xFF 空白（本脚本 §2 证实）
# 3. 字符串池：DATA_SEGMENT §2 已含 GBK 全表；本脚本 §3 GBK 判定误伤代码区，已废弃
import struct

BIN = r"D:\code\LPC1765FBD100\decompiled\LPC1765.bin"
b = open(BIN, "rb").read()
out = []
...
with open(r"D:\code\LPC1765FBD100\decompiled\docs\_sram_mirrors.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(out))
