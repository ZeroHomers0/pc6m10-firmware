...
| 10 | 程序关闭调试口 | `PINSEL1`（SWDIO/SWCLK=P0.27/P0.28 所在）全场 **0 次**访问；`PINSEL0` 3 处仅配低 16 脚（P0.6/P0.7），够不着 SWD 脚。CRP=PINSEL 均排除 |

**软件层至此穷尽。** 剩余只能靠硬件测量（§4）区分，且大概率落在「J-Link 能力不足」。

### 3.1 调试口关断排查（2026-08-24 附加）

用户怀疑程序初始化时关闭 SWD/JTAG。用 `arm-none-eabi-objdump` 反汇编原固件 +
二进制搜索外设地址字节验证：

| 寄存器 | 地址 | 出现次数 | 含义 |
|---|---|---|---|
| PINSEL1（管 SWDIO/SWCLK=P0.27/P0.28）| 0x4002C004 | **0** | 固件从未重配 SWD 引脚 |
| PINSEL0（管 P0.0–P0.15）| 0x4002C000 | 3 | 3 处 literal 引用，仅配低 16 脚（含 P0.6/P0.7 做普通 IO）|
| PINSEL10 / PINMODE0 / PINMODE1 | — | 0 | 未配置引脚上拉/下拉或特殊复用 |

**结论：原厂固件未通过 PINSEL/CRP 关闭调试口。** PINSEL0 改动只覆盖 P0.0–P0.15
（够不着 P0.27/P0.28 所在的 PINSEL1 辖区）；CRP=`0xFFFFFFFF` 无保护。故 SWD 在
固件侧是开放的，连不上更指向 J-Link 硬件兼容性或物理链路。
