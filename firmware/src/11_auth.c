 ...
 *   FIO 池 0x2009C000：auth_gpio_base=基址，+0x54 FIO2PIN、+0x58 FIO2SET、+0x5c FIO2CLR
 *   auth_timeout_counter_ptr=0x100020EC 认证超时窗口计数；auth_retry_state_ptr=0x10001730 重试状态机。
 *
...
{
  *auth_timeout_counter_ptr = 50000;
  return;
}
 ...
 *   延时 2000 → P2.3 时钟拉低（下降沿）→ 延时 1000 → bit_index>7 时读 P2.2（FIO2PIN&4）
 *   延时 1000 → P2.3 时钟拉高。
 * 挑战三字节在 bit_index==0/8/0x10 处生成写入 challenge_byte：
 *   uint8_t A = (验证码输入字段 1 + 字段 2 + 字段 3 + 字段 4 + 0x31) & 0xff
 *   uint8_t B = (验证码输入字段 5 + 字段 4 + 字段 6 + 0xc) & 0xff
 *   uint8_t C = 固定 0x55
 ...
 *   低字节比 exp_resp_lo；命中 → 清除超时与响应状态并置访问通过，
 *   否则 → 置响应失败并清除访问通过标志。
...
void auth_challenge(void)
{
  ...
  uint32_t exp_resp_hi;
  uint32_t exp_resp_lo;
  ...
  for (bit_index = 0; bit_index < 0x18; bit_index = bit_index + 1) {  /* 24 bit */
    ...
    if (bit_index == 8) {
      ...
      challenge_byte = (uint32_t)*parameter_current_limit_ptr + (uint32_t)*parameter_master_slave_offset_ptr + *parameter_phase_limit_ptr + 0xc;
  ...
  if ((exp_resp_hi == response_bits >> 8) && ((response_bits & 0xff) == exp_resp_lo)) {
    ...
    *system_access_granted_ptr = 1;
  }
  else {
    *auth_response_status_ptr = 1;                                                    /* 应答不匹配 → 认证失败 */
    *auth_response_byte_ptr = 1;
    *system_access_granted_ptr = 0;
  }
  return;
}

/* 0x00010820 —— 认证重试：仅当重试状态机 *auth_retry_state_ptr==1 时进入（开机认证未定）。
 *   每次 auth_challenge() 后若成功（*auth_response_status_ptr==0）→ 状态=2、重试计数=10（跳过）；
 *   若重试计数达到 4 仍失败 → 状态=2（放弃认证，允许继续运行；防锁死）。
 *   每次迭代末尾 param_sync_live_to_eeprom() 落盘参数。
 *   计数 *auth_retry_counter_ptr=0x100020F0（字节），pub：retry_counter_ptr。 */
void auth_retry(void)
{
  volatile uint8_t *retry_counter_ptr;

  retry_counter_ptr = auth_retry_counter_ptr;
  if (*auth_retry_state_ptr == 1) {
    *auth_retry_counter_ptr = *auth_retry_counter_ptr + 1;
    *retry_counter_ptr = 0;
    while (*auth_retry_counter_ptr < 5) {
      auth_set_timeout();
      auth_challenge();
      if (*auth_response_status_ptr == '\0') {                  /* 认证成功 */
        *auth_retry_state_ptr = 2;
        *auth_retry_counter_ptr = 10;
      }
      if (*auth_retry_counter_ptr == 4) {                     /* 已重试 4 次仍失败 */
        *auth_retry_state_ptr = 2;                          /* 放弃认证（允许继续运行） */
      }
