...
/* Chinese glyphs needed only by the language UI and absent from the factory font. */
static const unsigned char ext_char16_map[][2] = {
  {0xd3,0xef}, {0xd1,0xd4}, {0xd1,0xa1}, {0xd4,0xf1}, {0xce,0xc4} /* 语 言 选 择 文 */
};
...
void disp_render_char8(uint32_t ch,char row,uint32_t col,uint32_t invert)
{
  ...
  if (ch == 0x4f) {                  /* 'O'：直接复用原字库索引0的数字'0' */
    ...
    for (glyph_index = 0; glyph_index < sizeof(ext_char8_map) - 1U; ++glyph_index) {
      if (ext_char8_map[glyph_index] == ch) {
        glyph_base = (uint32_t)ext_char8_font;
        goto glyph_found;
      }
    }
    glyph_index = 0;
...
void disp_render_char16(uint32_t gb_hi,uint32_t gb_lo,char row,int col,uint32_t invert)
{
  ...
  for (glyph_index = 0; glyph_index < sizeof(ext_char16_map) / sizeof(ext_char16_map[0]); ++glyph_index) {
    if (ext_char16_map[glyph_index][0] == gb_hi && ext_char16_map[glyph_index][1] == gb_lo) {
      glyph_base = (uint32_t)ext_char16_font;
      goto glyph16_found;
    }
  }
  glyph_index = 0;
...
glyph16_found:
  ...
  for (bit_index = 0; bit_index < 0x10; bit_index = bit_index + 1 & 0xff) {
    ...
    disp_data(*(volatile uint8_t *)(glyph_base + glyph_index * 0x20 + bit_index),invert);
  }
  disp_cmd(row * '\x02' + -0x47);
  for (bit_index = 0; bit_index < 0x10; bit_index = bit_index + 1 & 0xff) {
    disp_cmd(col2 * 0x10 + 0x40 + bit_index & 0xff);
    disp_data(*(volatile uint8_t *)(glyph_base + glyph_index * 0x20 + bit_index + 0x10),invert);
  }
  return;
}
